...
class AdminUserView extends Controller {
    ...
    public function index() {
        ...
        /* Get the campaigns total */
        $user_campaigns_total = Database::$database->query("SELECT COUNT(*) AS `total` FROM `campaigns` WHERE `user_id` = {$user_id}")->fetch_object()->total ?? 0;

        /* Get the notifications total */
        $user_notifications_total = Database::$database->query("SELECT COUNT(*) AS `total` FROM `notifications` WHERE `user_id` = {$user_id}")->fetch_object()->total ?? 0;
        ...
        $data = [
            'user'                  => $user,
            'user_campaigns_total' => $user_campaigns_total,
