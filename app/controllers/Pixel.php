...
class Pixel extends Controller {
    ...
    public function index() {
        $seconds_to_cache = $this->settings->socialproofo->pixel_cache;
        ...
        \Altum\Language::set($user->language, false);
        ...
        $notifications_result = Database::$database->query("
            ...
            switch($notification->type) {
                case 'LATEST_CONVERSION':
                    ...
                    $result = Database::$database->query("
                        SELECT
                            `data`, `location`, `datetime`, `location`
                        FROM
                            `track_conversions`
                        WHERE
                    ...
                    if(!$result->num_rows) {
                        ...
                        while($conversion = $result->fetch_object()) {
                            ...
                            if($conversion->data) {
                                ...
                                $notification->last_action_date = $conversion->date;
                ...
                case 'CONVERSIONS_COUNTER':
                    ...
                    $notification->counter = Database::$database->query("
                        SELECT
                            COUNT(`id`) AS `total`
                        FROM
                            `track_conversions`
